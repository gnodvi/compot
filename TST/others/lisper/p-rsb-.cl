...
;(defconstant  RSB_SO  "T/r-rsb-.so")
;(defconstant  RSB_SO  "T/z-rsb-.so")
(defconstant  RSB_SO  "T/p-rsb-.so")
...
(defun l_set_last_history (num p1 p2)
  ...
  (when *simp_random*
    (format *error-output* "p1hist = ~s ~%"  p1hist)
    (format *error-output* "p2hist = ~s ~%"  p2hist)
    )
...
;  int  drawn = 0.05 * TRIALS; // т.е. ничья фиксируется в пределах 5% ;  
  ...
  (when *simp_random*
    (format *error-output* "~%rsb_play_match .. bot1= ~s  bot2= ~s  ~%~%" bot1 bot2))
...
;  // серия испытаний --------------------------------------------------
  (loop for i from 1 to TRIALS do
    ...
    (when *simp_random*
      (format *error-output*  "p1 = ~s  p2 = ~s ~%" p1 p2))
