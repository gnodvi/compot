...
(defun display-cells ()
  ...
  (if (and (>= (- (get-internal-real-time) *generation-timer*) 
               (* *generation-timer-timeout* internal-time-units-per-second)
               ) 
           (not *paused*)
           )
  ...
  (with-pushed-matrix
    ...
    (gl:with-primitives :line-strip
       ...
       (unless (null *extreme-points*)
         (let* (
           (lambda (lambda (way fun)(funcall fun (gol:extreme-coord *cells* way) 
                                             (+ 
...
(defun pixel->coords (left top)
  ...
  (multiple-value-list (glu:un-project left top 
                                       (aref (gl:read-pixels left top 1 1 :depth-component :float) 
                                             0)))
)
;-------------------------------------------------------------------------------
...
(let ((row (lambda() (list (make-list (length (first (slot-value *cells* 'cells-matrix))))))))
  ...
  (case way
    ...
    (:left  (setf (slot-value *cells* 'cells-matrix) 
                  (mapcar (lambda (item) (append (list nil) item)) (slot-value *cells* 'cells-matrix)))
            (incf (slot-value *cells* 'cells-x-out)))
    
    (:right (setf (slot-value *cells* 'cells-matrix) 
                  (mapcar (lambda (item) (append item (list nil))) (slot-value *cells* 'cells-matrix)))
            ))
  )
...
(defmethod glut:display ((w cube-window))
  ...
  (if *fix-coords*
      (gl:translate (- (- (/ (x-size *cells*) 2) (slot-value *cells* 'cells-x-out))) 
                    (- (/ (y-size *cells*) 2) (slot-value *cells* 'cells-y-out)) 0))
  (display-chooser)
  (display-cells)
  (display-borders)
...
(defun setxycell-by-chooser-coords ()
  ...
  (setxycell (first *chooser-coords*) (second *chooser-coords*) 
             *cells* (not (gol:xycell (first *chooser-coords*) (second *chooser-coords*) *cells*))
             ))
  ...
  (if (equal key #\Esc)
    ...
    (progn
      (case key
        ...
        (#\- (setf *generation-timer-timeout* (* *generation-timer-timeout* 2)) 
         (go-if-paused))
...
(defun run-game-of-life ()
  ...
  (glut:display-window  
   (make-instance 'cube-window  
                  :pos-x 200 :pos-y 400 :width 400 :height 400))
...
(defun run ()
  ...
  (let* (
    ...
    (*cells*  (make-live-cells glider6x6 ))       
    ;(*cells*  (make-live-cells))       
    )
