...
{ 
  ...
  case YPUSH:
    ...
    if (mes1 == hStop)      YKillTimer (tim); 
    if (mes1 == hContinue)  tim = YSetTimer (id, 10); 
    if (mes1 == hStep)  
      right_step (algorifm, numboids, world_w, world_h);
    break; 

  case YTIMER: 
    right_step (algorifm, numboids, world_w, world_h); // основной шаг
    break; 
...
{  
  ...
  case YDRAW: 
    ...
    YWnd (&hWinlist,  WINLIST,     "WINLIST",     5, top_h+middle_h+15, left_w, left_h, 
          0,0,0,0, CLR_DEF);    
    YWnd (&h_right,   right_proc,  "right_proc", 10+left_w, top_h+10, ww, WND_h-15-top_h, 
          0,0,0,0, CLR_DEF);    
    break; 
