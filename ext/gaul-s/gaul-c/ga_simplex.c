...
/*    schedule(static) nowait */
    for (i=1; i<num_points; i++)
      ...
      for (j=0; j<pop->len_chromosomes; j++)
        ((double *)putative[i]->CH)[j]
            = ((double *)putative[0]->CH)[j] +
              random_double_range(-pop->simplex_params->step,pop->simplex_params->step);
 ...
 */
    for (j = 0; j < pop->len_chromosomes; j++)
      ...
      for (i = 0; i < num_points-1; i++)
        average[j] += ((double *)putative[i]->CH)[j];
...
/*printf("DEBUG: average = ");*/
    ...
    for (j = 0; j < pop->len_chromosomes; j++)
      {
      if ( average[j]-TINY > ((double *)putative[pop->len_chromosomes]->CH)[j] ||
           average[j]+TINY < ((double *)putative[pop->len_chromosomes]->CH)[j] )
        restart_needed = FALSE;
   ...
   for (j = 0; j < pop->len_chromosomes; j++)
     ((double *)new1->CH)[j]
         = (1.0 + pop->simplex_params->alpha) * average[j] -
           pop->simplex_params->alpha * ((double *)putative[num_points-1]->CH)[j];
...
/*printf("DEBUG: new1 (%f) is fitter than p0 ( %f )\n", new1->fitness, putative[0]->fitness);*/
      ...
      for (j = 0; j < pop->len_chromosomes; j++)
        ((double *)new2->CH)[j]
            = (1.0 + pop->simplex_params->alpha) * ((double *)new1->CH)[j] -
              pop->simplex_params->alpha * ((double *)putative[num_points-1]->CH)[j];
      ...
      for (j = 0; j < pop->len_chromosomes; j++)
        ((double *)new1->CH)[j]
                = (1.0 - pop->simplex_params->beta) * average[j] +
                  pop->simplex_params->beta * ((double *)putative[num_points-1]->CH)[j];
            ...
            ((double *)putative[i]->CH)[j]
                = average[j] +
                  pop->simplex_params->gamma
                    * (((double *)putative[i]->CH)[j] - average[j]);
        ...
        for (i = 1; i < num_points; i++)
          ...
          for (j = 0; j < pop->len_chromosomes; j++)
            ((double *)putative[i]->CH)[j]
                = ((double *)putative[0]->CH)[j] +
                  pop->simplex_params->gamma
                    * (((double *)putative[i]->CH)[j] - ((double *)putative[0]->CH)[j]);
