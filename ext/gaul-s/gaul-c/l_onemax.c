...
boolean onemax_score(population *pop, entity *entity)
  ...
  for(i=0; i<NBITS; i++)
    //mgena    if(ga_bit_get((byte *)entity->CH, i) == 1) score += 1.0;
    if(ga_bit_get((gaulbyte *)entity->CH, i) == 1) score += 1.0;
...
int main(int argc, char **argv)
  ...
  for (i=0; i<NBITS; i++)
    //    printf("%d", ga_bit_get((byte *)ga_get_entity_from_rank(pop,0)->CH,i)?1:0);
    printf("%d", ga_bit_get((gaulbyte *)ga_get_entity_from_rank(pop,0)->CH,i)?1:0);
  printf("\n");
