...
static int Jim_ExecCmd(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
    ...
    // Gen:  TODO - ищем ошибку при вызове в QNX
    // . exec echo backslah backslash
    // 
    //printf ("Jim_ExecCmd (jim-exec.c) ... !!! argc = %d argv0= %s argv1= %s argv2= %s \n", 
    //        argc, 
    //        Jim_String(argv[0]), Jim_String(argv[1]), Jim_String(argv[2]));  
    ...
    // Gen:  - там надо искать ошибку двух слешей. хотя в исполнение уже
    // передается нормальный одлиночный слеш, видимо он и воспринимается как
    // команда печати имени папки программы?
    ...
    // Gen: 
    //printf ("Jim_ExecCmd ... errStrObj = %s \n", Jim_String (errStrObj));  
...
badargs:
    ...
    for (firstArg = 0; firstArg < arg_count; numPids++, firstArg = lastArg + 1) {
        ...
        if (pid < 0) {
          //
          // Gen: ошибка (в QNX) при при запуске некоторых команд из Си-программ,
          // например, Jim_Eval ->  "exec cp $SDIR/A/$FILE $SDIR/A/T"
          //
          //
        ...
        if (pid == 0) {
            ...
            // Gen: 
            // заменяет текущий образ процесса новым, выполняя
            // 
            //printf ("jim-exec.c: JimCreatePipeline: %s argv1 = %s \n", 
            //        arg_array[firstArg], arg_array[1]);
