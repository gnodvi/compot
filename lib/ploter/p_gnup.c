...
/*----------------------------------------------------------------------------*/
void
gnuplot_init_prepare (int w, int h, int x, int y)
{
...
gnuplot_init (void)
{
  ...
  // непосредственно запуск GNUPLOT (а как получить идентификатор окошка?)
  // handle->gnucmd = popen ("gnuplot", "w");

  printf ("gnuplot_init ... 1  \n");

  if (W == 0) sprintf (buff, "gnuplot -noraise");
  else {
    if (X < 0) sprintf (buff, "gnuplot -noraise -geometry %dx%d",       W, H);
    else       sprintf (buff, "gnuplot -noraise -geometry %dx%d+%d+%d", W, H, X, Y);
  }
  handle->gnucmd = popen (buff, "w");

  //  -raise   : raises plot window after each plot.
  //  -noraise : does not raise plot window after each plot.

  printf ("gnuplot_init ... 2  buff = %s \n", buff);
...
)
{
  ...
  /* Command to be sent to gnuplot    */
  if (handle->nplots > 0) strcpy (cmd, "replot");
  else                    strcpy (cmd, "plot");
  ...
  if (title == NULL) {
    ...
    sprintf (line, "%s \"%s\" title \"%s\" with %s %s", cmd, name, title, handle->pstyle,
             color);
  }
...
/* gnuplot> plot "/tmp/gnuplot-i-d54Vba" title "Sinus" with lines  */
/*               ^                                                 */
/*          can't read data file "/tmp/gnuplot-i-d54Vba"           */
/*          line 0: util.c: No such file or directory              */
 
// пока сигнал дошел до GNUPLOT, а файл уже удалили ?
...
)
{
  ...
  if (y==NULL) {
    ...
    gnuplot_plot_xy (handle, x, y, n, title, 1);
  }
...
gnuplot_i_example () 
{
  ...
  gnuplot_resetplot (h1);
  gnuplot_setstyle (h1, "points");
  gnuplot_plot_xy (h1, x, y, NPOINTS, "user-defined points", 1);
  sleep (SLEEP_LGTH);
...
gnuplot_i_sinepng ()
{
  ...
  gnuplot_cmd (g, "set output \"p_gnup.png\"");
