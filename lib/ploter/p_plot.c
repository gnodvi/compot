                       ...
                       double *p_xmin, double *p_xmax, double *p_ymin, double *p_ymax)
{
  ...
  //fprintf (stderr, "plot_get_minmax_frame: xmin=%f xmax=%f \n", xmin, xmax);
  //fprintf (stderr, "PP_XMIN= %f \n", P_XMIN(p, 0));
  //fprintf (stderr, "PP_XMAX= %f \n", P_XMAX(p, 0));
                    ...
                    int ti, YT_WIN  *win, YT_PLOT *plot, int x, int y, int h)
{
  ...
  if (plot->xnum <= h_all) { // помещаются все значения
    ...
    //fprintf (stderr, "h_all=%d  h_end=%d   h_beg=%d \n",  h_all, h_end, h_beg);
               ...
               int x, int y, int w, int h)
{
  ...
  int w2 = 21 + 10;
  //int w1 = w - w2+2;
  int w1 = w - w2 + 2; // 39 = 68 - 31 + 2

  //printf ("w= %d  w2= %d  w1= %d \n", w, w2, w1); //????!!!!
...
plot_step_beg (BOOL is_win, BOOL is_big,   YT_PLOT *plot)
{
  ...
  if (is_big) {
    ...
    //T_PLOTSTEP *
    //ps_big = plotbig_step_beg (plot, X, Y, W, H); 
