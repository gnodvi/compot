...
//mgena #elif defined(MICR_W) || defined(GCC__W) || defined(LCC__W) // Changes made by Ozerski 
#elif defined(GCC__W) 
...
/*------------------------------YSetXorMode-------------------------------*/ 
void 
YSetXorMode (YT_BOOL xorr) 
{ 
#if defined(API_W) 
  if (xorr)  SetROP2 (svw.hdc, R2_XORPEN); 
  else       SetROP2 (svw.hdc, R2_COPYPEN); 
#else  
  if (xorr)  XSetFunction (DPY, SV->gc, GXxor); 
