...
# Включение других приложений: (мтр. 685)
#
# ....
# 
# Приведенные ниже команды демонстрируют процесс запуска нового приложения
# wish и включения его окна в один из фреймов:
#
# frane .embed -container 1 -bd 4 -bg red
# exec wish somescript.tcl -use [winfo id .embed] &

#-------------------------------------------------------------------------------

# Tk has the ability to embed a toplevel inside another, or inside a frame. 
# The -use and -container options are used for this, along with winfo id. 
# For example:
...
# This can also work across processes on UNIX (and possibly Windows). The wish 
# application also takes a -use option which works in the same way (the root "." 
# window becomes embedded).
...
proc testForSounds {} {
  ...
  foreach {snd file} {sndShot shot.wav sndExplosion explosion.wav \
    ...
    set file [file join $::TOPDIR/M/$file] ;# перенесем в отдельную папку!
...
proc initVars {} {
  ...
  #puts "sw= $sw   sh= $sh" ;# sw= 1920   sh= 1080

  set ::SMALL [expr {$sw < 800}]

  if {$::SMALL} {
    set ::SCALE [expr {$sw / 800.0}] ;# уменьшаем масштаб, чтобы
    set ::globals(screenWidth)  $sw  ;# рисовать на всем маленьком экране
    set ::globals(screenHeight) $sh

  } else {
    set ::SCALE 1 ;# это "родной размер" анимации
  ...
  if {[info exists ::starkit::topdir]} {
    # что это пока не понял !
    #
    set ::globals(highScoreFile) \
      [file join [file dirname $::starkit::topdir] "asteroids_hs.txt"]
  } else {
    #puts "not exist ::starkit::topdir !!!"
    set ::globals(highScoreFile) [file join $::TOPDIR "M/asteroids_hs.txt"]
  }
...
proc buildUI {{root .}} {
  ...
  # - т.е. надо, чтобы это как-то реально изменялось     !!
  #
  #wm resizable . 0 0
}
#-------------------------------------------------------------------------------
