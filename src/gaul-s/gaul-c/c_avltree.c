...
References: 
   ...
   Just about any datastructure text book.
...
To do:	rebuilding tree after changing key/hashing function.
 	...
 	convert some recursive functions into iterative functions where 
        sensible (e.g. avltree_node_count)
...
static AVLNode	*avltree_node_remove_leftmost (AVLNode *node, AVLNode **leftmost);
static AVLNode	*avltree_node_restore_left_balance (AVLNode *node, int old_balance);
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_new (AVLKey key, vpointer data)
{
  AVLNode *node;
...
/*----------------------------------------------------------------------------*/
static void 
avltree_node_free (AVLNode *node)
{
  THREAD_LOCK (avltree_node_buffer_lock);
...
/*----------------------------------------------------------------------------*/
static void 
avltree_node_delete (AVLNode *node)
{
...
/*----------------------------------------------------------------------------*/
static void 
avltree_node_destroy (AVLNode *node, AVLDestructorFunc free_func)
{
...
/*----------------------------------------------------------------------------*/
static boolean 
avltree_node_search (AVLNode *node,
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_insert (AVLNode *node,
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_remove (AVLNode *node,
                     AVLKey key, vpointer *removed_data)
{
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_balance (AVLNode *node)
{
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_remove_leftmost (AVLNode  *node, AVLNode **leftmost)
{
  int old_balance;
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_lookup_leftmost (AVLNode *node)
{
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_lookup_rightmost (AVLNode *node)
{
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_restore_left_balance (AVLNode  *node,
				  int	     old_balance)
{
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_restore_right_balance (AVLNode *node,
				   int	     old_balance)
{
...
/*----------------------------------------------------------------------------*/
static vpointer 
avltree_node_lookup(AVLNode *node, AVLKey key)
{
  if (!node) return NULL;
...
/*----------------------------------------------------------------------------*/
static boolean 
avltree_node_traverse (AVLNode *node,
                       AVLTraverseFunc traverse_func, vpointer userdata)
{

  if (node->left!=NULL) {
    if (avltree_node_traverse (node->left, traverse_func, userdata)) 
      return TRUE;
  }

  if ((*traverse_func)(node->key, node->data, userdata)) 
    return TRUE;

  if (node->right!=NULL) {
    if (avltree_node_traverse(node->right, traverse_func, userdata)) 
      return TRUE;
  }
...
/*----------------------------------------------------------------------------*/
static int 
avltree_node_height (AVLNode *node)
{
  int left_height;
  int right_height;
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_rotate_left (AVLNode *node)
{
  AVLNode *right;
  int a_bal;
  int b_bal;
...
/*----------------------------------------------------------------------------*/
static AVLNode *
avltree_node_rotate_right (AVLNode *node)
{
  AVLNode *left;
  int a_bal;
  int b_bal;
...
/*----------------------------------------------------------------------------*/
static void 
avltree_node_check (AVLNode *node)
{
...
/*----------------------------------------------------------------------------*/
GAULFUNC AVLTree *
avltree_new (AVLKeyFunc key_generate_func)
{
  AVLTree *tree;
...
/*----------------------------------------------------------------------------*/
GAULFUNC void 
avltree_destroy (AVLTree *tree, AVLDestructorFunc free_func)
{
  if (!tree) return;

  if (free_func!=NULL) avltree_node_destroy (tree->root, free_func);
  else                 avltree_node_delete  (tree->root);
...
/*----------------------------------------------------------------------------*/
GAULFUNC boolean 
avltree_insert (AVLTree *tree, vpointer data)
{
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_remove (AVLTree *tree, vpointer data)
{
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_remove_key (AVLTree *tree, AVLKey key)
{
  vpointer removed=NULL;
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_lookup (AVLTree *tree, vpointer data)
{
  AVLNode	*node;
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_lookup_key (AVLTree *tree, AVLKey key)
{
  AVLNode  *node;
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_lookup_lowest (AVLTree *tree)
{
  AVLNode  *node;
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_lookup_highest(AVLTree *tree)
{
  AVLNode  *node;
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_ordered_search (AVLTree *tree,
                        AVLSearchFunc search_func, vpointer userdata)
{
  if (!tree || !tree->root) return NULL;
...
/*----------------------------------------------------------------------------*/
GAULFUNC vpointer 
avltree_search (AVLTree *tree, AVLMatchFunc search_func, vpointer userdata)
{
...
/*----------------------------------------------------------------------------*/
GAULFUNC void 
avltree_traverse (AVLTree *tree, AVLTraverseFunc traverse_func, vpointer userdata)
{
...
/*----------------------------------------------------------------------------*/
GAULFUNC int 
avltree_num_nodes(AVLTree *tree)
{
...
/*----------------------------------------------------------------------------*/
static AVLKey
test_avltree_generate (constvpointer data)
{
...
/*----------------------------------------------------------------------------*/
static boolean 
test_avltree_traverse (AVLKey key, vpointer data, vpointer userdata)
{
  /* check. */
